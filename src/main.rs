...
fn provide_input(parser: &mut Parser<String>, 
    ...
    while !consumed {
        ...
        consumed = parser.consume(terminal_token_rule_element.clone(), &terminal_value, &grammar_state_hashmap);
        *step = *step + 1;
    }
...
fn consume_character(dfa: &mut EpsilonNfa::<State, RegexBuildingBlock>, 
    ...
    grammar_state_hashmap: &BTreeMap<usize, GrammarState<String>>) -> usize {
...
/*
    ...
    for character in str.chars() {
        ...
        current_state_id = consume_character(&mut dfa, current_state_id, &mut token_string_buffer, current_character, lookahead_character, &mut step, &mut parser, &grammar_state_hashmap);
    }

    current_state_id = consume_character(&mut dfa, current_state_id, &mut token_string_buffer, lookahead_character, 'x', &mut step, &mut parser, &grammar_state_hashmap);
...
*/
    ...
    provide_input(&mut parser, &grammar_state_hashmap, &mut step, &RuleElement::Terminal(dfa.states[&current_state_id].token_name.clone()), &token_string_buffer);
    provide_input(&mut parser, &grammar_state_hashmap, &mut step, &RuleElement::Closure, &token_string_buffer);
